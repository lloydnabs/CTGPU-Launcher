...
void HandleMultiModPacks(u64 titleID/*,bool showMenu*/) {
   ...
   for (std::map<std::string,std::string>::iterator it=mounting_points.begin(); it!=mounting_points.end(); ++it){
		...
		std::experimental::filesystem::copy(modTitleIDPath + "/CTGP-U", modTitleIDPath + "/MyStuff", std::experimental::filesystem::copy_options::skip_symlinks, std::experimental::filesystem::copy_options::recursive, std::experimental::filesystem::copy_options::overwrite_existing);
    ...
    while(1){
        ...
        if(initScreen) {
            OSScreenClearBufferEx(0, 0);
            OSScreenClearBufferEx(1, 0);
            console_print_pos(x_offset, -1, "                  -- CTGP-U Launcher by Lloyd Nabbit --");
            console_print_pos(x_offset, 1, "Based on SDCafiine by Maschell");
            int y_offset = 3;
            int cur_ = 0;
